...
function DeleteEvent() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const resG = await axios.get(`http://localhost:4000/Api/V1/Event/${id}`, config);
        setEvent(resG.data)
        console.log(resG.data)
        console.log(resG.data.Moderateur)
        const resG2 = await axios.get(`http://localhost:4000/Api/V1/Admins/${resG.data.Moderateur}`, config);
  ...
  const handleDelete = async () => {
    try {
      setError(false);
      const res = await axios.delete(`http://localhost:4000/Api/V1/Event/${id}`, config);
