...
Cypress.Commands.add("loginadmin", (email, password) => {
  ...
  cy.get('input[data-test="email"]').type(email);
  cy.get('input[data-test="password"]').type(password);
  cy.get('button[data-test="login-button"]').click(); // Assuming the button has a data-test attribute // Adjust the URL to the redirected page after login
});
...
Cypress.Commands.add("addAlumni", (email, password) => {
  ...
  cy.intercept("POST", "http://localhost:4000/Api/V1/register").as(
    "registerRequest"
  );
